...
double contourFunctionRastrigin(double x, double y) {
    double contour = 20 + pow(x, 2) + pow(y, 2) - 10 * (cos(2 * 3.14 * x) + cos(2 * 3.14 * y));
    return contour;
}
...
int main(int argc, char *argv[]) {
    ...
    //change to true to have console output
    bool consoleOutputFlag = false;
    int iterationHop;
    //change 4th argument to "y" to write to file
    bool writeFileFlag;
    //change to 1 for basic contour and 2 for Rastrigin
    int chooseContourFunction = 1;
    ...
    if (numThreads <= 10) {
        iterationHop = 20;//cuts time in half instead of 1
    ...
    //this creates a 2d array of numParticles particles and 3 columns per particle each row is used for an x,y,z
    ...
    for (int i = 0; i < numParticles; i++) {
        ...
        }else if (chooseContourFunction == 2) {
    ...
    for (int i = 0; i < numIters; i++) {
        ...
        for (int j = 0; j < numParticles; j++) {
            ...
            if (threadIDs > 2) {//fix leter all the threads are rounting to the else cause
                ...
                double z;
                if (chooseContourFunction == 1) {
                    z = contourFunction(particle(j,0), particle(j,1)); //get z of the particle at that x and y
                }else if (chooseContourFunction == 2) {
                    z = contourFunctionRastrigin(particle(j,0), particle(j,1)); //get z of the particle at that x and y
                }else {
                    z = contourFunction(particle(j,0), particle(j,1)); //get z of the particle at that x and y
                }
